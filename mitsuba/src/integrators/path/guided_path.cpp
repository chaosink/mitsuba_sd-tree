...
public:
    GuidedPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
        ...
        m_lfBlockX = props.getInteger("lfBlockX", -1);
        m_lfBlockY = props.getInteger("lfBlockY", -1);
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        if(m_lfSampleX != -1 && m_lfSampleY != -1 && m_lfBlockX != -1 && m_lfBlockY != -1) {
            ...
            if(m_lfSampleX != -1 && m_lfSampleY != -1)
                lfSampleRecord.WriteEXRDirSpace(m_lfSampleX, m_lfSampleY);
            if(m_lfBlockX != -1 && m_lfBlockY != -1)
                lfSampleRecord.WriteEXRImageSpace(m_lfBlockX, m_lfBlockY);
        }
    ...
    void renderBlock(const Scene *scene, const Sensor *sensor,
        ...
        // NNAdaptive: shading normal buffer
        ref<ImageBlock> normalBlock = new ImageBlock(Bitmap::ESpectrumAlpha, block->getSize(), block->getReconstructionFilter());
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            for (int j = 0; j < m_sppPerPass; j++) {
                ...
                squaredBlock->put(samplePos, radiance * radiance, rRec.alpha);
                sampler->advance();
            }
        }
                    ...
                    if (!value.isZero()) {
                        ...
                        if (!m_strictNormals || woDotGeoN * Frame::cosTheta(bRec.wo) > 0) {
                            ...
                            if (dTree) {
                                ...
                                v.commit(false); // 直接光采样的光场不计入path vertex总数
                            }
...
private:
    ...
    int m_lfSampleX, m_lfSampleY;
    int m_lfBlockX, m_lfBlockY;
